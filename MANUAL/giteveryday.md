...
- [git-pull(1)](git-pull.html) and [git-fetch(1)](git-fetch.html) from "origin" to keep up-to-date with the upstream.
...
satellite$ git config remote.origin.push \
 +refs/heads/_:refs/remotes/satellite/_ (3)
...
- [git-daemon(1)](git-daemon.html) to allow anonymous download from repository.

- [git-shell(1)](git-shell.html) can be used as a _restricted login shell_ for shared central repository users.
...
Run git-daemon to serve /pub/scm from xinetd.  
 $ cat /etc/xinetd.d/git-daemon # default: off # description: The Git server offers access to Git repositories
service git
{
disable = no
type = UNLISTED
