...
They are both passed directly to *git diff-tree*; see [git-diff-tree(1)](git-diff-tree.html) for more information.
...
Since *dcommit* uses rebase internally, any Git branches you *git push* to before *dcommit* on will require forcing an overwrite of the existing ref on the remote repository. This is generally considered bad practice, see the [git-push(1)](git-push.html) documentation for details.
